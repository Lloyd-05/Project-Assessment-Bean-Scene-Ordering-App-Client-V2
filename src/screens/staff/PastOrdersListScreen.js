...
const PastOrdersListScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    if (!result) {
      ...
      navigation.navigate("pastOrder", { date: result.date });
    }
  };
        ...
        {pastOrders.map((order, index) => (
          <View key={index} style={[styles.orderCard, { backgroundColor: theme.colors.beanLightBlue }]}>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('pastOrder', { date: order.date })}
            >
...
export default PastOrdersListScreen;
